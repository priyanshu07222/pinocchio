...
pub struct WithdrawAccouts<'a>{
    pub vault: &'a AccountInfo,
    pub owner: &'a AccountInfo,
    pub bump: [u8;1]
}

impl<'a> TryFrom<&'a [AccountInfo]> for WithdrawAccouts<'a> {
    ...
    fn try_from(accounts: &'a [AccountInfo]) -> Result<Self, Self::Error> {
        ...
        Ok(Self { vault, owner, bump: [bump] })
    }
}

pub struct Withdraw<'a> {
    pub accounts: WithdrawAccouts<'a>
}
...
impl<'a> TryFrom<&'a [AccountInfo]> for Withdraw<'a>{
    ...
    fn try_from(accounts: &'a [AccountInfo]) -> Result<Self, Self::Error> {
        let accounts = WithdrawAccouts::try_from(accounts)?;
